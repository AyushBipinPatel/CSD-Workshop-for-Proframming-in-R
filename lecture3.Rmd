---
title: "Building your masterpiece with {ggplot2}"
subtitle: "Lecture 3"
author: "Ayush Patel"
...
# Building your masterpiece using

<img src="https://raw.githubusercontent.com/rstudio/hex-stickers/master/thumbs/ggplot2.png" width="150px"/>
...
.tiny[ggplot2 logo from rstudio hex sticker git page]
---
name: Introduction
...
<br>
--
You know __how to subset rows from a data frame.__
<br>
<br>
<br>
--
You know __how to mutate a data frame.__
---
...
## Dont we need to install the ggplot2 library?
...
background-image: url(figs/concept_map_visualization.jpg)
...
```{r comp_plot, eval=FALSE, include=FALSE}
  ...
  geom_boxplot(aes(species, body_mass_g,
                   fill = species),
               alpha = 0.5)+ # geom and aesthetic
  geom_jitter(aes(species,body_mass_g,
                  colour = species),
              alpha = 0.5)+ # another layer with aesthetics
  theme_bw()+ # theme
  labs(
    x = "Species",
...
Make a histogram of a continuous variable of your choice from the penguins data. __Plot is also an object__. Save it as you would save a vector. __Name it `chart_density`__
...
```{r time, echo=FALSE}
countdown(minutes = 4,play_sound = TRUE,bottom = 0)
```
...
Make a scatter plot of two continuous variables of your choice from the penguins data. __Name it `chart_scatter`__
...
## Plotting one continuous anf one discrete variable
...
Copy the following commands in console and execute one after another.

>install.packages("nycflights13")
>ibrary(nycflights13)

Use the `View()` function to see the `flights` data.

__Make a chart that you think will show some interesting insight.__
