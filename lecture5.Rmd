---
title: "Implementing Statistical Tools in R"
subtitle: "Lecture 5"
author: "Ayush Patel"
...
# Implementing Statistical Tools in R
...
<br>
--
You know __What is a Hypotheses and the difference between a research hypotheses and a statistical hypotheses__
<br>
<br>
<br>
--
You know __t-tests conceptually__
<br>
<br>
<br>
--
You know __Chi square-tests conceptually__
<br>
<br>
<br>
--
You know __ANOVA conceptually__
<br>
<br>
<br>
--
You know __Linear regression conceptually__
<br>
<br>
<br>
---
...
### Learning Statistics with R by Dr. Danielle Navarro

This lecture is stolen from her book.
---
...
.panel[.panel-name[Choice1]
```{r echo=FALSE} 
table(cards$choice_1) -> observed
observed
```

]<!----panel 2----->
.panel[.panel-name[Null H]
H0: All four suits are chosen with equal probability<br>
H0: P = ( .25, .25, .25, .25)
]
.panel[.panel-name[Alternative H1]
H1: At least one of the suit-choice probabilities isn’t .25<br>
H1: P != ( .25, .25, .25, .25)

]
.panel[.panel-name[Observed and Expected]
...
.panel[.panel-name[Critical Value]
```{r}
qchisq(p = 0.95,df = 3) # df = k-1
...
.panel[.panel-name[Silver bullet]
```{r}
goodnessOfFitTest(cards$choice_1) # the function is from the lsr package

# one could also us the funciton chisq.test()
```
...
.panel[.panel-name[One Sample]
```{r}
oneSampleTTest(x = grades, mu = 67.5)
```

]
.panel[.panel-name[Independent Samples test - Data]
```{r echo=FALSE}
harpo %>%
knitr::kable()%>%
kableExtra::kable_styling()%>%
...
.panel[.panel-name[Independent Samples test - Code]
```{r }
...
.panel[.panel-name[Drug and mood gain]
```{r echo=FALSE}
clin.trial %>% 
  group_by(drug) %>% 
  summarise(
    mean.mood.gain = mean(mood.gain, na.rm = T)
  )
```
...
```{r}
aov(formula = mood.gain ~ drug, data = clin.trial) -> anova_test
